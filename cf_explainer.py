...
class CFExplainer:
    ...
    def __init__(
            self, model, graph_ae, sub_adj, sub_feat, n_hid, dropout, lr, n_momentum, cf_optimizer,
                 sub_labels, y_pred_orig, num_classes, beta, device, algorithm='loss_PN_AE_', edge_additions=False, kappa=10):
        ...
        self.cf_model = GCNSyntheticPerturb(
            ...
            self.num_classes, self.sub_adj, dropout, beta, edge_additions=edge_additions
        )
    ...
    def train_cf_model(self, epoch):

        output, output_actual, y_pred_new, y_pred_new_actual = self.forward_cf_model()
        if self.algorithm == 'cfgnn':
            loss_total, loss_perturb, loss_graph_dist, cf_adj = self.cf_model.loss(
                output[self.new_idx], self.y_pred_orig, y_pred_new_actual
            )
        if self.algorithm == 'loss_PN_L1_L2':
    ...
    def explain(
        ...
        for epoch in range(num_epochs):
            new_example, loss_total = self.train_cf_model(epoch)
