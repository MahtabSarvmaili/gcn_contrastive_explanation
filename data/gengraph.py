...
import data.synthetic_structsim as synthetic_structsim
import data.featgen as featgen
...
def save_gen_graph(G, labels, path=None):
    num_nodes = G.nodes.__len__()
    feat_dim = G.nodes.data()[0]['feat'].__len__()
    adjacency_mrx = torch.zeros(num_nodes, num_nodes)
    features_mrx = torch.zeros(num_nodes, feat_dim)
    # feature matrix
    for i, dt in enumerate(G._node.items()):
        features_mrx[i, :] = torch.from_numpy(dt[1]['feat'])
    # adjacency matrix
    for dt in G.adjacency():
        for x in dt[1].keys():
