...
class Node_Explainer:
    ...
    def explain(self, node_idx, num_samples=100, top_node=None, p_threshold=0.05, pred_threshold=0.1):
        ...
        A_torch = dense_to_sparse(A_torch)[0]
        X_torch = X_torch.squeeze(dim=0)
        pred_torch = self.model.forward(X_torch, A_torch)
        ...
        for iteration in range(num_samples):
            ...
            X_perturb_torch = X_perturb_torch.squeeze(dim=0)
            pred_perturb_torch = self.model.forward(X_perturb_torch, A_torch)
